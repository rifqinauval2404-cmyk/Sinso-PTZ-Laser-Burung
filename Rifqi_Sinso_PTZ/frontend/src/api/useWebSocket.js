...
export function useWebSocket() {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      // WebSocket needs a fully-qualified URL (unlike fetch, it can't take a relative path),
      // so derive the current page's directory from location.pathname at runtime instead of
      // assuming root "/" - works both at "/" (local) and behind a reverse-proxy sub-path
      // like "/pkl/<project>/" (see PANDUAN_PKL.md), since the proxy forwards the WS request
      // to this same origin/path prefix before stripping it for the backend.
      const basePath = location.pathname.replace(/[^/]*$/, "");
      const wsUrl = `${proto}://${location.host}${basePath}ws?key=${encodeURIComponent(API_KEY)}`;
