...
> **Catatan (2026-07-10)**: Isi file ini di bawah menceritakan era `hmi.html`+`server.js` (sekarang diarsipkan ke folder `sinso bawaan PLN/` ini). Arsitektur AKTIF sekarang ada di `frontend/` (React/Vite) + `backend/` (Node/Express + MySQL) di root project - lihat `CLAUDE.md` di root untuk deskripsi terkini. Dua update penting yang tetap relevan lintas versi:
> 1. **IP device berubah**: `192.168.1.60` → **`10.8.242.50`** (dipindah ke jaringan perusahaan/TJB untuk keperluan deploy ke server PKL). Port Pelco-D (`4196`) tidak berubah.
> 2. **Device punya web admin panel sendiri**, terpisah total dari kontrol Pelco-D di atas - diakses langsung lewat browser ke IP device (mis. `http://10.8.242.50/`), dipakai untuk ubah setting jaringan device (termasuk IP-nya). Ini bukan bagian dari aplikasi kita, dan tidak ada hubungannya dengan temuan "device auto-kembali ke posisi preset (H≈94, V≈38) setelah idle 45-50 detik tanpa command gerak" (itu soal memori POSISI MOTOR, bukan MEMORI KONFIGURASI JARINGAN - dua sistem berbeda di firmware yang sama).
> 3. **Diuji (2026-07-13) dan TERBUKTI TIDAK AMAN: Pelco-D standard "Call Preset" (cmd2=`0x07`, Data2=nomor preset).** Device TIDAK berhenti di posisi preset tersimpan seperti seharusnya - malah bergerak TERUS-MENERUS di kedua sumbu (H naik ~pan-right, V turun ~tilt-down) tanpa berhenti sendiri, sampai dikirim `stop` manual (diuji: H bergerak 94→120→194 dan V 38→29→10 dalam ~7 detik sebelum dihentikan paksa). Kesimpulan: **device ini TIDAK mengimplementasikan Set/Call/Clear Preset Pelco-D standar dengan benar** - kemungkinan clone/implementasi parsial. **JANGAN kirim command Preset (`0x03`/`0x05`/`0x07`) atau command lain di luar tabel "confirmed live" di bawah tanpa pengawasan ketat + siap kirim `stop` segera** - device bisa merespons dengan gerakan tak terduga/tak terkendali, bukan gagal-aman diam. Posisi setelah tes ini kembali aman di H≈94/V≈38 (posisi idle normal).
